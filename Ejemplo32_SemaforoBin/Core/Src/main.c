...
//  SEGGER_SYSVIEW_Start();
  ...
  configASSERT(status == pdPASS);
  status= xTaskCreate(vTask3, "tarea3", configMINIMAL_STACK_SIZE, NULL, 0, xTask2);
  configASSERT(status == pdPASS);
...
BaseType_t status;
	...
	for (;;) {
		...
		SEGGER_SYSVIEW_PrintfHost("Activa el Semaforo con USERBUTTON");
		while(HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13));
		status = xSemaphoreGive(xSemphore);
		if (status == pdTRUE) {
			SEGGER_SYSVIEW_PrintfHost("Semaforo Libre");
			HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);
		}else{

			SEGGER_SYSVIEW_PrintfHost("Semaforo Fail");
		}
...
void vTask2(void *params){
	...
	for(;;){
		...
		status = xSemaphoreTake(xSemphore,pdMS_TO_TICKS(1000));
		if (status == pdTRUE) {
...
void vTask3(void *params){
	...
	for(;;){
		status = xSemaphoreTake(xSemphore,pdMS_TO_TICKS(2000));
		if (status == pdTRUE) {
