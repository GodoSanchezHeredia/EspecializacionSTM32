...
BaseType_t estado;
	...
	for (;;) {
		...
		if (estado ==pdTRUE) {
			SEGGER_SYSVIEW_PrintfHost("Dato Enviado");
		}else {
...
for(;;){
	...
	estado = xQueueReceive(xQueue, (void *)&var, 500);
	if (estado ==pdTRUE) {
				SEGGER_SYSVIEW_PrintfHost("Dato recibido :%u",var);
			}else {
				SEGGER_SYSVIEW_PrintfHost("Dato no disponivke");
			}
